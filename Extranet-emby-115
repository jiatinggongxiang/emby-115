...
echo "你需要一个已部署且可以访问的emby（可以非本机），且本机已安装好docker。脚本开发：SMZDM:sonatasss 大佬。一键脚本by NetSE"
...
read -p "请输入原emby内网地址及端口，可在emby后台查看，默认：http://172.17.0.1:8096（除非确有必要，请不要改IP），如不理解，请直接回车；如果emby非本机安装，请输入上一步相同地址：" new_emby_loc && \
